    ...
    Public Shared Async Function CreateLogin(ByVal LoginPicture As Bitmap) As Task(Of Loginstatus)
        ...
        Try
            ...
            Else
                ...
                'Dim domaimSetup As AppDomainSetup = New AppDomainSetup
                'domaimSetup.ApplicationBase = AppDomain.CurrentDomain.BaseDirectory
                'Dim domain As AppDomain = AppDomain.CreateDomain("zakidomain", Nothing, domaimSetup)


   
                'Dim instance As Object = domain.CreateInstanceAndUnwrap("LoginDLL, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null", "BasicLogin")
                ...
                'Dim assem As Assembly = domain.Load(AssemblyName.GetAssemblyName(DllSource))
                'Dim type_ As Type = assem.GetType("BasicLogin")
                'Dim myClassInstance As Object = Activator.CreateInstance(type_)
                'Dim method As MethodInfo = type_.GetMethod("BasiHashData")
                ''method.Invoke(myClassInstance, Nothing)
                'AppDomain.Unload(domain)

                'Dim dom As AppDomain = AppDomain.CreateDomain("MyNewDomain")


                'Dim myClassType As Type = Assembly.LoadFile(DllSource).GetType("BasicLogin")

                'If myClassType IsNot Nothing Then

                '    Dim myClassInstance As Object = Activator.CreateInstance(myClassType)
                '    ' dom.CreateInstance("LoginDLL, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null", "BasicLogin") 
